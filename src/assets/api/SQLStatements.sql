...
-- SQL for Invoice to get data per filter params - CompanyId, SowId, DateFrom and DateTo
	select CONCAT(Users.First_Name, ' ', Users.Last_Name) As Full_Name, Company.Company_Name, Company.Company_Id, Project.Project_Category, Project.Sow_Id, Timesheets.Sub_Assignment, Timesheets.Sub_Assignment_Segment_1, SUM(Timesheets.Task_Hours) As Total_Hours from v_kash_operations_timesheet_table_date as Timesheets join kash_operations_created_projects_table as Project on timesheets.sow_id = Project.sow_id join kash_operations_user_table as Users on Timesheets.Emp_Id = Users.Emp_Id join kash_operations_company_table as Company on Project.Company_Id = Company.Company_Id where (Timesheets.Entry_Date >=cast(? as date) AND Timesheets.Entry_Date <=cast(? as date)) AND (Timesheets.non_billable_reason IN ('n/a','N/A','')) AND (Company.Company_ID = cast(?)) AND (Project.Sow_Id=cast(?)) Group By Full_Name, Timesheets.Sub_Assignment, Timesheets.Sub_Assignment_Segment_1, Company.Company_Name, Company.Company_Id, Project.Project_Category, Project.Sow_Id Order by Full_Name
...
select CONCAT(Users.First_Name, ' ', Users.Last_Name) As Full_Name, Company.Company_Name, Company.Company_Id, Project.Project_Category, Project.Sow_Id, Timesheets.Sub_Assignment, Timesheets.Sub_Assignment_Segment_1, SUM(Timesheets.Task_Hours) As Total_Hours from v_kash_operations_timesheet_table_date as Timesheets join kash_operations_created_projects_table as Project on timesheets.sow_id = Project.sow_id join kash_operations_user_table as Users on Timesheets.Emp_Id = Users.Emp_Id join kash_operations_company_table as Company on Project.Company_Id = Company.Company_Id where (Timesheets.Entry_Date >=cast(? as date) AND Timesheets.Entry_Date <=cast(? as date)) AND (Timesheets.non_billable_reason IN ('n/a','N/A','')) AND (Company.CompanyId = cast(? as char)) Group By Full_Name, Timesheets.Sub_Assignment, Timesheets.Sub_Assignment_Segment_1, Company.Company_Name, Company.Company_Id, Project.Project_Category, Project.Sow_Id Order by Full_Name
