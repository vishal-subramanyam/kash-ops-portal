...
function ProjectSubCategory(props) {
  ...
  let [tasksBySubCategory, setTaskBySubCategory] = useState(props.allSubCategories.filter((task) => {
    return task.SowId === props.subCategory.SowId && task.ProjectSubTaskId === props.subCategory.ProjectSubTaskId && task.Segment1 !== "" && task.Segment1 !== "-"
  }))
  ...
  const areYouSure = () => {
   ...
   const deleteSubCategory = async (sowId, subCatId) => {
    ...
    try {
      ...
      console.log(`Deleted user ${props.
      subCategory.SubTaskTitle}`, data);
                ...
                <details className="main-grouping">
                    ...
                    <div className="workspace-add-task-to-workarea-input">
                      <form className="workspace-add-task-input-area">
                          ...
                          type="button"
                          onClick={() => addTaskToSubCategory(props.projectId, props.subCatTitle, props.subCatId, taskSegment1.current.value)}>Add Task</button>
                      </form>
                    </div>
                    <div id="segment1id">
                        {tasksBySubCategory.map((task) => {
                            return <SubCategoryTask subCategoryTask={task} 
                    ...
                    <div id="confirmmsgdiv" className="modal-dialog modal-confirm">
                          <div className="modal-content">
                              ...
                              <div id="removeconfirmpopup" className="modal-body">
                                  <p>Are you sure you want to delete <b>{props.subCategory.SubTaskTitle}</b>? </p>    				
                              </div>
                              <div className="modal-footer justify-content-center">
                                  ...
                                  <button type="button" className="modal-btn btn-danger" onClick={() => deleteSubCategory(props.projectId, props.subCatId)}>Delete</button>
                              </div>
                          </div>
                      </div>
                  </dialog>
